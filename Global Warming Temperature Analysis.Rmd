...
temp_data <- read.csv(file = 'Temperature.csv')
...
```{r}
Summer_ts = subset(Total_temp_ts,cycle(Total_temp_ts) %in% c(6,7,8))
Fall_ts = subset(Total_temp_ts, cycle(Total_temp_ts) %in% c(9,10,11))
Winter_ts = subset(Total_temp_ts, cycle(Total_temp_ts) %in% c(12,1,2))
Spring_ts = subset(Total_temp_ts, cycle(Total_temp_ts) %in% c(3,4,5))
